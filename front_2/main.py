...
async def upload(file: UploadFile = File(...)):
    ...
    segments, info = model.transcribe(
        ...
        condition_on_previous_text=False, # reduces error snowball
        temperature=[0.0, 0.2, 0.4],      # stability
    ...
    for seg in segments:
        if seg.words:
            ...
            # Fallback if no word-level info: split segment text evenly
            ...
            if tokens:
                ...
                for i, tok in enumerate(tokens):
                    ...
                    e = s + max(0.08, step * 0.9)
                    words.append({"t": tok, "s": s, "e": e})
    ...
    print(f"[VerseMotion] duration={duration:.2f}s, words={len(words)}, lines={len(lines)}")
